...
- parent `bundle exec rake test`: **36/36 PASS** (新規 4 spec 含む、 detail は test/fluent/test_filter_natural_language_mac.rb)
- swiftcap `swift test`: **9/9 PASS** (touch なし)
- mini-E5: 本変更は config + filter 1 行 + encoding tag のみ、 readiness/timing layer に影響無いため skip 判断 (前バージョン baseline 4/5 維持を期待値)
- 30s `say -v Kyoko` 検証 (filler-rich text):
   - 入力: 'うん、ちょっとですね、ありがとうござい、リリース品質確認のテストです。' / 'なるほど、まあ、けど、じゃあ会議の議事録を作成しますね。'
   - new entities=20、 new edges=11
   - top entities 全て content (あり / まし / こと / さん / しか / しかも / たくさん / わし / デイズ / フォー / 初日 / 本当 / 来る / 楽しみ / 正直 / 残り / 確信)
   - **stopwords leak 件数 = 0** (26 stopwords を `IN (...)` 句で全数 check、 `COUNT=0`)
- Chrome `localhost:9292`: 3 ペイン (Quick / Perfect / Graph) 全描画、 canvas 1877x947、 「こんにちは」 verbatim 出現

### 制覇までの軌跡

実装中、 期待した 1 commit (config-only) → 検証 0 hit が判明 → BCP-47 lookup miss を fix (Round B) → それでも 0 hit → fluentd の `-Eascii-8bit:ascii-8bit` 由来の encoding mismatch を発見 (Round C)。 最初の Round C fix (`File.read encoding: UTF-8`) が transcoding crash を引き起こし fluentd worker 落ちる事故。 force_encoding 方式に切替えて成功。 release 品質水準への hidden regression を 3 段で完全除去。
